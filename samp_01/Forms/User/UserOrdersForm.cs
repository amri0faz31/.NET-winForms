...
namespace samp_01.Forms.User
{
    public class UserOrdersForm : Form
    {
        ...
        public UserOrdersForm(UserProfileDTO user, string scope = "active")
        {
            _user = user; _scope = scope;
            Text = scope.Equals("completed", StringComparison.OrdinalIgnoreCase) ? "Completed Orders" : "My Orders";
        ...
        private void LoadOrders()
        {
            ...
            foreach (var o in filtered)
            {
                ...
                var card = new Panel { Width = 320, Height = (_scope == "completed" ? 140 : 170), BackColor = Color.White, BorderStyle = BorderStyle.FixedSingle, Margin = new Padding(10), Padding = new Padding(12) };
                var title = new Label { Text = o.Title, Left = 0, Top = 0, Width = 280, Font = new Font("Segoe UI", 10, FontStyle.Bold) };
                var sub = new Label { Text = $"Seller: {seller?.CompanyName ?? ("Seller #" + o.SellerId)}\nStatus: {o.Status}\nPrice: {o.TotalPrice:0.##}", Left = 0, Top = 28, Width = 280, Height = 60 };
        ...
        private void OpenConversation(Order order)
        {
            ...
            var sellerName = _sellers.GetSellerEntityById(order.SellerId)?.CompanyName ?? ($"Seller #{order.SellerId}");
            Action reload = () =>
            {
                ...
                foreach (var m in _messages.GetByOrder(order.Id))
                {
                    ...
                    string whoName = m.SenderType.Equals("Seller", StringComparison.OrdinalIgnoreCase) ? sellerName : _user.Name;
